...
class Dashboard(models.Model):
    ...
    parent_menu_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref(
            "mba_bi_dashboard.parent_dashboard_menu"
        ),
        domain=[("action", "=", False)],
        tracking=True,
    )
            ...
            cron_id = self.env["ir.cron"].create(
                {
                    ...
                    "model_id": self.env.ref(
                        "mba_bi_dashboard.model_dashboard_dashboard"
                    ).id,
                    "state": "code",
    ...
    def action_view_charts(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "mba_bi_dashboard.dashboard_chart_action"
        )
        if len(chart_ids) > 1:
            ...
            form_view = [
                (
                    self.env.ref("mba_bi_dashboard.dashboard_chart_form_view").id,
                    "form",
                )
            ]
    ...
    def get_charts_details(self):
        ...
        if (
            user
            and user.has_group("mba_bi_dashboard.group_dashboard_user")
...
class DashboardMail(models.Model):
    ...
    def _default_mail_template(self):
        ...
        """
        if self.env.ref("mba_bi_dashboard.mail_template_dashboard_chart"):
            return self.env.ref(
                "mba_bi_dashboard.mail_template_dashboard_chart"
            ).id
