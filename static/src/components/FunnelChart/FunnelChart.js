...
export class FunnelChart extends Component {
  static template = "mba_bi_dashboard.FunnelChart";
  static props = {
    chartId: String,
    name: String,
  ...
  render_funnel_chart() {
    ...
    const container = document.getElementById("funnel_chart__" + this.props.chartId);
    ...
    const option = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          return `<strong>${params.name}</strong><br/>${params.marker} Conversión: <strong>${val}</strong> (${params.percent}%)`;
        },
      },
      legend: {
        bottom: 0,
      ...
      series: [
        {
          name: this.props.name || "Funnel Chart",
          type: "funnel",
          left: "10%",
          top: "8%",
          bottom: "16%",
    ...
    this.chartInstance.on("click", (params) => {
      if (this.props.update_chart && params.data) {
        this.props.update_chart(parseInt(this.props.chartId), "funnel_chart", {
